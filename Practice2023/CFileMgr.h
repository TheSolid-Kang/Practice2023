...
public:
...
#if UNICODE 
				str_file_path = file_path.wstring();//std::filesystem::path -> std::wstring 으로 변환
#else
				file_path(file_path.string());//std::filesystem::path -> std::string 으로 변환
#endif;
	...
	static std::list<std::string> GetRecursiveFilesInDirectoryTemp(const std::string& _path) {
		...
		while (true != iter._At_end()) {
			try {
				std::filesystem::path file_path = (*iter++).path();
				str_file_path = file_path.string();//std::filesystem::path -> std::string 으로 변환
